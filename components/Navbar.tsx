...
export default function Navbar() {
  ...
  return (
    <>
      {/* 桌面侧边栏 - 可收起 */}
      <nav 
        className={`hidden md:flex fixed left-0 top-0 bottom-0 z-50 flex-col items-center justify-between bg-white border-r border-slate-200 py-6 transition-all duration-300 overflow-hidden ${
      ...
      {/* 移动端顶部导航栏 */}
      <nav className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Rocket size={24} className="text-blue-600" />
          <span className="text-slate-900 font-semibold">Web DL Manager</span>
        </div>
      ...
      {/* 移动端菜单下拉框 */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed top-14 left-0 right-0 z-40 bg-white border-b border-slate-200 shadow-xl animate-in fade-in slide-in-from-top-2 duration-200">
