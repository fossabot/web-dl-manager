...
-   **灵活上传:** 利用 `rclone` 将存档上传到各种云存储提供商 (WebDAV, S3, B2) 或 `gofile.io`。**对于 `openlist` 服务，压缩和未压缩的上传都直接由 `openlist.py` 处理，以实现无缝集成。当上传到包含“terabox”的目录时，将不会创建额外的 `task_id` 子目录。**
-   **实时日志:** 重新设计的状态页面实时显示任务日志，无需页面刷新，并包含复制到剪贴板的功能。
-   **服务器状态页面:** 在 `/server-status` 路径下提供一个专门的页面，用于实时监控系统资源（CPU、内存、磁盘）、应用程序正常运行时间、活动任务数以及关键依赖项的版本。
